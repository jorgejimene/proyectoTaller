...
void Legendre(int n, int m, double fi, Matrix &pnm, Matrix &dpnm) {
    ...
    for (int i = 2; i <= n; i++) {
        pnm(i + 1, i + 1) = sqrt(static_cast<double>(2 * i + 1) / (2 * i)) * cos(fi) * pnm(i, i);
    }
    for (int i = 2; i <= n; i++) {
        dpnm(i + 1, i + 1) = sqrt(static_cast<double>(2 * i + 1) / (2 * i)) * ((cos(fi) * dpnm(i, i)) -(sin(fi) * pnm(i, i)));
    }
    /* horizontal
    first
    step coefficients
    ...
    */
    ...
    while (1) {
        for (int i = k; i <= n; i++) {
            pnm(i + 1, j + 1) = sqrt(static_cast<double>(2 * i + 1) / ((i - j) * (i + j))) *((sqrt(2 * i - 1) * sin(fi) * pnm(i, j + 1))
    ...
    while (1) {
        for (int i = k; i <= n; i++) {
            dpnm(i + 1, j + 1) = sqrt(static_cast<double>(2 * i + 1) / ((i - j) * (i + j))) *((sqrt(2 * i - 1) * sin(fi) *
                                                                            dpnm(i, j + 1))
